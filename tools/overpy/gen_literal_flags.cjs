// Regenerates crates/opy-rs/src/compiler/size_optimization/literal_flags.rs from
// the pinned OverPy 9.7.10 argument tables (`canReplace0ByFalse`,
...
setTimeout(() => {
  ...
  for (const [name, info] of Object.entries(globalThis.__funcKw())) {
    ...
    (info.args ?? []).forEach((arg, index) => {
      ...
      if (flags.length) rows.push(`    ("${canonical}", ${index}, ${flags.join(" | ")}),`);
    });
  }
  rows.sort();
  const out = path.join(__dirname, "../../crates/opy-rs/src/compiler/size_optimization/literal_flags.rs");
  fs.writeFileSync(
    out,
