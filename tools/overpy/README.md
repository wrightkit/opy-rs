# OverPy Evidence Harness

This directory contains the reproducible OverPy evidence harness described by
...
```text
crates/opy-rs/tests/fixtures/corpus/<category>/<name>/
...
closures. See `crates/opy-rs/tests/fixtures/corpus/README.md` for the complete provenance
...
[`crates/opy-rs/tests/differential-expectations.json`](../../crates/opy-rs/tests/differential-expectations.json)
is the independent native-side expectation record. Each fixture has a native
outcome, an expected relationship to the oracle (`match`, `known-gap`, or
`unsupported`), a rationale, and evidence.
...
exact contract. Complete Workshop numeric tokens outside quoted strings are
compared by their exact numeric value. A token starts with a digit and
consumes digits and `.`, so lexical spellings such as `0` and `0.0` are
equivalent while `1e3`, `.5`, and a value change such as `0.0` to `0.5` remain
regressions. Exact-output reporting still exposes presentation differences.
...
python3 tools/overpy/run_native.py \
  --binary target/debug/opy-cli \
  --semantic-binary target/debug/opy-compat
python3 tools/overpy/diff.py --results /path/to/results --report target/opy-rs-differential-report.json
```
