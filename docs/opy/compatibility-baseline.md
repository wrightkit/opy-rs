...
| 5a | `#!mainFile`, `#!allowMacroRedeclaration`, `#!optimize*`/`#!replace0By*` family, `#!translations`, `#!rulePrefix*`, `__script__` JS hooks | `baseline-supported` for source implementation state; optimizer/locale/hook effects remain lowering-dependent | ✅ | ✅ | partial | ✅ | ✅ oracle probes |
