...
The baseline records seven independent workloads:

| Workload | Mechanism isolated | Input and provenance |
| --- | --- | --- |
| `nested-expression-lowering` | Nested lowering values and subtree copies | Generated source, depth 24, SHA-256 recorded in output |
| `wide-call-value-construction` | Wide value construction and nested argument copies | Generated `[vect(...)]`, 32 vectors, SHA-256 recorded in output |
| `many-rule-compilation` | Retained lowering state across rules | Generated source, 128 rules, SHA-256 recorded in output |
| `compiler-contract-initialization` | Repeated manifest/catalog contract initialization | Generated `Compiler::new()` workload, 64 constructions, SHA-256 recorded in output |
| `macro-runtime` | QuickJS engine creation per invocation | Fixed script `(x + 2).toString();`, 64 invocations, SHA-256 recorded in output |
| `large-settings-source` | Settings scanner materialization | Generated JSONC-like block with 1,000 keys, SHA-256 recorded in output |
